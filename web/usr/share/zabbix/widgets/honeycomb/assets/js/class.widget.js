...
class CWidgetHoneycomb extends CWidget {
  ...
  onResize() {
    ...
    if (
      this.#items_max_count > old_items_max_count &&
  ...
  setContents(response) {
    if (this.#honeycomb === null) {
      ...
      this.#honeycomb
        .getSVGElement()
        .addEventListener(CSVGHoneycomb.EVENT_CELL_CLICK, (e) => {
          this.#selected_hostid = e.detail.hostid;
          this.#selected_itemid = e.detail.itemid;
          this.#selected_key_ = this.#cells_data.get(
            this.#selected_itemid,
          ).key_;

          const $obj = jQuery(e.detail.trigger_element);
          const mouse_event = e.detail.mouse_event;

          // 기존에 열려 있는 팝업 메뉴 닫기.
          jQuery(".menu-popup-top").menuPopup("close", null, false);

          const url = new Curl("zabbix.php");

          url.setArgument("action", "menu.popup");
          url.setArgument("type", "host");
          ...
          jQuery
            ...
            .done((resp) => {
              ...
              const sections = getMenuPopupHost(resp.data, $obj);

              /*
               * Web Monitoring Overview를 Host 기본 메뉴에 추가.
               */
              const view_section = sections.find(
                (section) => section.label === t("View"),
              );

              if (view_section !== undefined) {
                const dashboard_url = new Curl("zabbix.php");

                dashboard_url.setArgument("action", "host.dashboard.view");
                dashboard_url.setArgument("hostid", this.#selected_hostid);
                dashboard_url.setArgument("dashboardid", "407");

                view_section.items.splice(1, 0, {
                  label: "Web Monitoring Overview",
                  url: dashboard_url.getUrl(),
                });
              }

              $obj.menuPopup(sections, new jQuery.Event(mouse_event));
            })
            .fail((xhr, status, error) => {
              console.error("Failed to load host menu:", status, error);
            });
        });

      this.#honeycomb
        .getSVGElement()
        .addEventListener(CSVGHoneycomb.EVENT_CELL_ENTER, (e) => {
      ...
      this.#honeycomb
        .getSVGElement()
        .addEventListener(CSVGHoneycomb.EVENT_CELL_LEAVE, (e) => {
    ...
    if (
      ...
      if (
        this.#selected_itemid === null ||
        (!this.#hasSelectable() && !this.#selectItemidByKey())
      ) {
  ...
  getActionsContextMenu({ can_copy_widget, can_paste_widget }) {
    const menu = super.getActionsContextMenu({
      can_copy_widget,
      can_paste_widget,
    });
    ...
    for (const search_menu_actions of menu) {
      if (
        "label" in search_menu_actions &&
        search_menu_actions.label === t("Actions")
      ) {
  ...
  #getItemsMaxCount() {
    ...
    const { max_rows, max_columns } = CSVGHoneycomb.getContainerMaxParams({
      width,
      height,
    });
